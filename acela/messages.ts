...
abstract class AbstractCommand {
    ...
    toJSON(): object {
        const byteOffset = Math.ceil(Math.log(this.getCommandValue()) / Math.log(256)) - 1;
        const data = this.getCommandValue() & (Math.pow(256, byteOffset) - 1);

        return {
            opCode: this.opCode,
            data: this.getCommandValue() < 256 ? null : data,
        };
    };
...
export const factoryCreateCommand = (opCode: number, data: number|null = null): AbstractCommand => {
    if (opCode === OpCode.ACTIVATE_CONTROL) {
        return new ActivateCommand(data!);
    } else if (opCode === OpCode.READ_VERSION) {
        return new ReadRevisionCommand();
    }
...
export class ActivateCommand extends AbstractCommand {
    ...
    constructor(controlAddress: number) {
        super();
